...
uint32_t data_length)
{
	...
	volatile enum status_code sc_a = can_set_tx_buffer_element(&can_instance, &tx_element, buffer);
	volatile enum status_code sc_b = can_tx_transfer_request(&can_instance, 1 << buffer);
	volatile int a = 1;
}
...
int main (void)
{
	...
	struct system_pinmux_config pconfig;
	system_pinmux_get_config_defaults(&pconfig);
	pconfig.direction = PORT_PIN_DIR_OUTPUT;
	system_pinmux_pin_set_config(LED_0_PIN, &pconfig);
	system_pinmux_pin_set_config(LED_1_PIN, &pconfig);
	...
	while(1) {
		...
		port_pin_set_output_level(LED_1_PIN, ledstate);
		ledstate = !ledstate;
