...
void configure_can(void)
{
	...
	config_can0.automatic_retransmission = false;
...
void can_send_standard_message(uint32_t id_value, uint8_t *data, uint32_t data_length)
{
	...
	can_set_tx_buffer_element(&can0_instance, &tx_element, buffer);
	can_tx_transfer_request(&can0_instance, 1 << buffer);
}
...
uint32_t data_length)
{
	...
	volatile enum status_code sc_a = can_set_tx_buffer_element(&can0_instance, &tx_element, buffer);
	volatile enum status_code sc_b = can_tx_transfer_request(&can0_instance, 1 << buffer);
	volatile int a = 1;
}