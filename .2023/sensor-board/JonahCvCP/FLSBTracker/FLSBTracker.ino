...
unsigned long SPITracker[1000];
unsigned int SPICount = 1;
unsigned long CANTracker[1000];
unsigned int CANCount = 1;

unsigned long OGCANTime = millis();
unsigned long OGSPITime = millis();
...
void setup() {
  ...
  SPITracker[0] = OGSPITime;
  
  CANTracker[0] = OGCANTime;
}
...
void loop() {
  ...
  CANTracker[CANCount] = millis();
  CANCount++;

  if(CANCount ==999){
    for(int i=0; i<1000; i++){
      SPITracker[i]-=OGSPITime;
      CANTracker[i]-=OGCANTime;
    }
    Serial.print("SPI TRACKER: [");
    for(int i=0; i<1000;i++){
      Serial.print(SPITracker[i]);
      Serial.print(",");
    }
    Serial.println("]");
    Serial.print("CAN TRACKER: {");
    for(int i=0;i<1000;i++){
      Serial.print(CANTracker[i]);
      Serial.print(",");
    }
  }
