...
const float factor[LEN] = {
  ...
  0.01, 0.01, 0.01, 0.01,  .001, 0.00024414, 0.00024414, .001, 1, 1, 1, 1
};
const float offset[LEN] = {
...
void loop() {
  ...
  if (len != (LEN*2)) return;                // expect full frame
  ...
  for (uint8_t i = 0; i < LEN; ++i) {
    ...
    if (i < (LEN-1)) Serial.print(',');      // comma-separate, no trailing comma
  }
  Serial.println();
}
